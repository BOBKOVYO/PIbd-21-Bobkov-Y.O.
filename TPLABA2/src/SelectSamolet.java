...
public class SelectSamolet extends JDialog {

	Itechnica samolet = null;
	JPanel panel;
	boolean r;

	public SelectSamolet(JFrame parent) {
		super(parent, true);
		main();
	}
	...
	public void main() {
		...
		JLabel lblSamolet = new JLabel("Samolet");
		lblSamolet.setBounds(10, 29, 58, 14);
		this.getContentPane().add(lblSamolet);

		JLabel lblfrontovoibombardir = new JLabel("frontovoibombardir");
		lblfrontovoibombardir.setBounds(10, 64, 160, 14);
		this.getContentPane().add(lblfrontovoibombardir);
		...
		lblSamolet.addMouseListener(mouseL);
		lblfrontovoibombardir.addMouseListener(mouseL);
		lblfrontovoibombardir.setTransferHandler(new TransferHandler("text"));
		lblSamolet.setTransferHandler(new TransferHandler("text"));
			...
			public void drop(DropTargetDropEvent e) {
				...
				try {
					...
					for (DataFlavor df : e.getTransferable().getTransferDataFlavors()) {
						if (e.getTransferable().getTransferData(df) == "Samolet") {
							samolet = new Samolet(100, 4, 1000, Color.WHITE);
						} else if (e.getTransferable().getTransferData(df) == "frontovoibombardir") {
							samolet = new frontovoibombardir(100, 4, 1000, Color.WHITE, true, true, Color.BLACK);
						}
						draw(panel, samolet);
					}
				} catch (Exception ex) {
		...
		JLabel lblDopColor = new JLabel("Доп цвет");
		...
		lblMainColor.setDropTarget(new DropTarget() {
			...
			public void drop(DropTargetDropEvent e) {
				if (samolet != null) {
					try {
						for (DataFlavor df : e.getTransferable().getTransferDataFlavors()) {
							samolet.setMainColor((selectColor(e.getTransferable().getTransferData(df).toString())));
							draw(panel, samolet);
						}
					} catch (Exception ex) {
			...
			public void drop(DropTargetDropEvent e) {
				if (samolet != null) {
					try {
						...
						for (DataFlavor df : e.getTransferable().getTransferDataFlavors()) {
							((frontovoibombardir) samolet).setdopColor((selectColor(e.getTransferable().getTransferData(df).toString())));
							draw(panel, samolet);
						}
					} catch (Exception ex) {
						System.out.println(ex);
	...
	public Itechnica getSamolet() {
		return samolet;
	}

	public void draw(JPanel panel, Itechnica samolet) {
		if (samolet != null) {
